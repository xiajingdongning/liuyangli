...
void main()
{
    ...
    vec3 lightDir = normalize(pointLights[7].lightPos - FragPos);
    // Now we can compute the diffuse light impact
    float diffImpact = max(dot(norm, lightDir), 0.0);
    vec3 diffuseLight = diffImpact * pointLights[7].lightColor;

    vec3 Lighting = diffuseLight;
    vec3 viewPos = vec3(0.0,0.0,0.0);
    vec3 viewDir = normalize(viewPos - FragPos);
    vec3 result = Lighting+diffuseColor*CalcPointLight(pointLights[7], norm, FragPos, viewDir);
