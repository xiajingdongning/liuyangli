import mongodb from 'mongodb';

let favoritesCollection;
const ObjectId = mongodb.ObjectId;

export default class FavoritesDAO {
  static async injectDB(conn) {
    if (favoritesCollection) {
      return;
    }
    try {
      favoritesCollection = await conn.db(process.env.CITIESREVIEWS_COLLECTION).collection('favorites');
    } catch (e) {
      console.error(`Unable to establish a collection handle in FavoritesDAO: ${e}`);
    }
  }

  static async updateFavorites(userId, favorites) {
    try {
        console.log('update Favorites', favorites)
        // return await favoritesCollection.insertOne({_id: new ObjectId(userId), test: 'test'});
      const updateResponse = await favoritesCollection.updateOne(
        { _id: userId },
        { $set: { favorites: favorites }},
        { upsert: true }
      )

      console.log('update Favorites', updateResponse)
      return updateResponse;
    } catch(e) {
      console.error(`Unable to update favorites: ${e}`);
      return { error: e };
    }
  }

  static async getFavorites(id) {
    let cursor;
    try {
      cursor = await favoritesCollection.find({ _id: id });
      const favorites = await cursor.toArray();
      return favorites[0] || [];
    } catch(e) {
      console.error(`Something went wrong in getFavorites: ${e}`);
      throw e;
    }
  }
}